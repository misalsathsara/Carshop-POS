...
// Basic validation
if (empty($id) || empty($name) || !is_numeric($cost_price) || !is_numeric($price) || !is_numeric($selling_price) || empty($warranty) || !is_numeric($stock)) {
...
try {
    ...
    $stmt = $conn->prepare("UPDATE products SET name=?, cost_price=?, price=?, selling_price=?, warranty=?, stock=? WHERE id=?");
    if (!$stmt) {
        throw new Exception("Prepare failed: " . $conn->error);
    }
    $stmt->bind_param("sdddsss", $name, $cost_price, $price, $selling_price, $warranty, $stock, $id);
    if (!$stmt->execute()) {
